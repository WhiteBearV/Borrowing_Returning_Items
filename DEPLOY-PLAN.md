...
> อัปเดต 18 ส.ค. 2026: QA ครบวงจรรอบใหม่บนเครื่อง dev เจอ+แก้บั๊ก dashboard formula, 500 crash ตอนกางกลุ่มอุปกรณ์, และบั๊กชุดอุปกรณ์ (bundle) ยืมซ้ำ — **แก้ข้อมูลชุดอุปกรณ์ที่พังไปแล้วบน DB dev เท่านั้น** เพิ่มข้อ 13 ในรายการ "ยังต้องทำบน VM จริง" เพราะ DB จริงจะว่างเปล่า (สร้างจาก `alembic upgrade head`) ไม่มีชุดอุปกรณ์เดิมมาด้วย
...
- [ ] 13. **(18 ส.ค.) ตั้งค่าชุดอุปกรณ์ (bundles) ใหม่ทั้งหมด** — DB จริงว่างเปล่า ไม่มีชุดอุปกรณ์ติดมาจาก dev เลย (bundles อยู่ในข้อมูล ไม่ใช่ schema/migration) ต้องเข้าหน้า "จัดการอุปกรณ์" → แก้ไขอุปกรณ์ตัวกระตุ้น → ติ๊ก "ตัวกระตุ้น" ให้เอง อย่างน้อย 3 ชุดที่เคยตั้งไว้บน dev:
  - **ชุดUnoR3** — ตัวกระตุ้นต้องเป็น **Arduino-UNO-R3** (ของจริง ไม่ใช่ Arduino Mega ที่เคยตั้งผิดตัวรอบแรกบน dev) + item = สายUSB-C
  - **Pi 4** — ตัวกระตุ้น Raspberry Pi 4 Computer + items = power supply/MicroSD/card reader/keyboard/mouse/monitor
  - **ชุดคอมพิวเตอร์ตั้งโต๊ะ Dell Optiplex 7040** — ตัวกระตุ้น Dell Optiplex + items = mouse/keyboard/สายไฟ
  - ระวัง: **ห้ามติ๊กอุปกรณ์ตัวกระตุ้นให้ "รวมในชุด" ตัวเองด้วย** (บั๊กเดิมที่เพิ่งแก้) — ตอนนี้มี guard กันไว้ทั้ง backend (400 ถ้าส่ง API ตรง ๆ) และ UI (checkbox กันติ๊กพร้อมกัน) แล้ว แต่ยังต้องตั้งค่าถูกตัวอยู่ดี
