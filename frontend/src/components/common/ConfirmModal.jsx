      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onCancel}
            className="flex-1 rounded-full border border-gray-200 py-2 text-sm text-gray-600 hover:bg-gray-50">
            ยกเลิก
          </button>
          <button onClick={onConfirm}
            className={`flex-1 rounded-full py-2 text-sm font-semibold text-white ${danger ? 'bg-red-600 hover:bg-red-700' : 'bg-primary-600 hover:bg-primary-700'}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
